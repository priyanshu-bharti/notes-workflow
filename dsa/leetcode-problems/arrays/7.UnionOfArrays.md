...
difficulty: "Easy"
description: "Given two sorted arrays nums1 and nums2, return an array that contains the union of these two arrays. The elements in the union must be in ascending order."
use-cases: "Merging sorted datasets, removing duplicates, two-pointer pattern"
time-complexity: "O(n + m) where n = len(nums1), m = len(nums2)"
space-complexity: "O(n + m) for the result array (in worst case when all elements are unique)"
naive-approach: "Merge both arrays, use set() to remove duplicates, then sort. Avoid this in interviews as sorting adds extra time."
optimal-approach: "Use two-pointer traversal to merge while skipping duplicates on the fly. No extra sorting needed. Arrays must be sorted for two-pointer to work. Carefully skip duplicates during merge to avoid repeated values."
---
