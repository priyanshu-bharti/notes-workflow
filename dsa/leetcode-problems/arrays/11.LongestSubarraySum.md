...
topic: "Arrays"
difficulty: "Easy"
problem: "Given an array and a sum k, print the length of the longest subarray that sums to k."
description: "Given an integer array and a target sum k, return the length of the longest contiguous subarray that sums up to k. The array may contain negative numbers."
use-cases: "Prefix sum problems, sliding window limitations, subarray-based questions"
time-complexity: "O(n)"
space-complexity: "O(n)"
naive-approach: "Use two nested loops to check all possible subarrays and calculate their sums. If a subarray sum equals k, update the max length."
optimal-approach: "Use a prefix sum with a hashmap to store first occurrences of cumulative sums. For each index, check if (prefix_sum - k) exists in the map to find a valid subarray. Don’t update the hashmap if a prefix sum appears again — store only the first occurrence to ensure the longest subarray is found. Also, a sliding window won’t work if the array has negative numbers."
---
